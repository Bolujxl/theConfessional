...
void main() {
  ...
  gl_FragColor  = color;
      ...
      const loop = (t: number) => {
        ...
        if (followMouse && mouseInfluence > 0.0) {
          ...
          (uniforms.mousePos.value as [number, number]) = [
            smoothMouseRef.current.x,
            smoothMouseRef.current.y,
          ];
        }
  ...
  useEffect(() => {
    ...
    (u.raysColor.value as [number, number, number]) = hexToRgb(raysColor);
    ...
    (u.rayPos.value as [number, number]) = anchor;
    (u.rayDir.value as [number, number]) = dir;
  }, [
    raysColor,
    raysSpeed,
    lightSpread,
