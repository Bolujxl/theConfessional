...
const ConfessionCard = memo(function ConfessionCard({ confession, now, opacity = 1 }: Props) {
  const shouldReduceMotion = useReducedMotion();

  return (
    // Outer div carries the ephemeral fade — separate from Framer Motion's entrance animation.
    <div style={{ opacity, transition: 'opacity 1s linear' }}>
      <motion.div
        id={confession.id}
